...
function submitForm() {
	...
	if(inDetectorSeq != dictNfcCodeSeq){
		$.messager.alert('提示','探测器NFC序列号对应的编号与输入的探测器编号不一致,不能提交!');
		return;
	}
	var isStart = 'N';
	var isEnd = 'N';
	...
	$.ajax( {
	    ...
	    success:function(data) {
	    	if(data!=null && data.length > 0){
	    		var item = data[0];
	    		if(item.start_point_id != null && item.start_point_id != '' && item.start_point_id !='<%=id%>'){
	    			//启点探测器已分配
	    			$("#chkStartPoint").attr("disabled",true);
	    			$("#spanStartPointTip").text("已有探测器设置为起点.");
	    		}
	    		if(item.end_point_id != null && item.end_point_id != '' && item.end_point_id !='<%=id%>'){
	    			//终点探测器已分配
	...
	$.ajax( {
	    ...
	    success:function(data) {
	    	if(data!=null && data.length > 0){
	    		...
	    		$("#gcjLongitude").textbox('setValue',processorObj.gcj_longitude);
	    		$("#gcjLatitude").textbox('setValue', processorObj.gcj_latitude);
	    		if(processorObj.start_point == 'Y'){
	    			$("#chkStartPoint").attr("checked",true);
	    		}
	    		if(processorObj.end_point == 'Y'){
	    			$("#chkEndPoint").attr("checked",true);
	    		}
	    	}
	    },
	    error : function(data) {
	    ...
	    onChange: function(value) {
	    	...
	    	if(objValue.length == 14){
	    		$.ajax( {
				    ...
				    data:{codeName:'detector',codeValue:objValue,id:'<%=id%>'},
		    		type:'post',
				    async:false,
				    dataType:'json',
				    success:function(data) {
