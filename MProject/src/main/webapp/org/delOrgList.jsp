	...
	<table id="dg" class="easyui-datagrid"  
		...
		<thead>
			<tr>
				...
				<th data-options="field:'org_name',width:100,sortable:true">单位名称</th>
				<th data-options="field:'tax_number',width:150,sortable:true">单位税号</th>
...
<script>
	...
	function showButtons(val,row){
		var columnItem = '<span><a href="javascript:void(0)" class="easyui-linkbutton" onclick="doRecover(\''+val+'\')" style="width:80px;">恢 复</a></span>&nbsp;&nbsp;';
		return columnItem;
	}
	
	function doRecover(val){
		$.messager.confirm('确认', '确认恢复该单位?', function(r){
			if(r){
				$.ajax( {
				    ...
				    success:function(data) {
				    	if(data.returnCode == "success"){
				    		parent.loadUrl("<%=path%>/org/delOrgList.jsp");
				    	}else{
